...
from icescopy_frame_source import frame_source_from_preview_payload
...
class VideoPreviewDecodeWorker(QObject):
    ...
    def __init__(self, frame_source_payload, preview_cache_dir, source_token, parent=None):
        super().__init__(parent)
        self.frame_source_payload = dict(frame_source_payload or {})
    ...
    def decode(self, index):
        ...
        try:
            ...
            if self.frame_source is None:
                self.frame_source = frame_source_from_preview_payload(
                    self.frame_source_payload,
            ...
            self.failed.emit(requested_index, str(err), self.source_token)
            return
        if not self.closed:
            self.decoded.emit(index, q_image, frame_key, self.source_token)
