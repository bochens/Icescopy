...
Use single-cell and grid tools to mark droplets, wells, or other assay locations directly on the image data. Icescopy pairs these annotations with a robust grayscale-based freeze finding workflow, while visible location numbers keep manual review, freezing calls, and exported results tied to the same physical locations.
